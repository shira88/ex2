...
bool available_ids[MAX_THREAD_NUM] = {false};
...
void jump_to_thread ()
{
  ...
  {
    std::cout << "system error: setitimer error." << std::endl;
  }
...
int uthread_spawn (thread_entry_point entry_point)
{
  ...
  {
    std::cout << "thread library error: NULL entry point" << std::endl;
    return -1;
  }
...
int uthread_terminate (int tid)
{
  ...
  {
    std::cout << "thread library error: No such thread" << std::endl;
    return -1;
  }

  // check if the tid is in the ready queue or the blocked, and if so
  // removes it
...
int uthread_block (int tid)
{
  ...
  if (!is_valid_tid (tid))
  {
    std::cout << "thread library error: invalid tid\n";
    return -1;
  }
  //CHECKS IF IT ISN'T ALREADY BLOCKED
  if (blocked_set.find (tid) != blocked_set.end ())
  {
    std::cout << "thread library error: tid is already blocked\n";
    return -1;
  }
...
int uthread_sleep (int num_quantums)
{
  ...
  if (running_thread == 0)
  {
    std::cout << "Cannot call uthread_sleep from the main thread";
    return -1;
  }
